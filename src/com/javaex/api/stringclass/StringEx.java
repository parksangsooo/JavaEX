...
//        stringBasic();
    ...
    private static void usefulMethods(){
        ...
        index = str.indexOf("Programming");     // str내에 "Programming"의 위치 인덱스 출력
        ...
        index += "Programming".length(); // 검색 시작 위치를 뒤로 이동
        index = str.indexOf("Programming", index);      // 새로운 시작 위치에서 검색
        System.out.println("3rd search: " + index);     // 더 이상 검색할 것이 없으면 "-1" 출력
        // indexOf 로 검색했을 때, 찾는 것이 없으면 -1 을 반환한다
        ...
        // 부분 문자열 추출 : substring
        System.out.println("substring: " + str.substring(5));   // 5번 index 부터 끝까지 추출
        System.out.println("substring: " + str.substring(5, 16));   // 5번 index 부터 16번째 앞까지 추출

        System.out.println("---------------------------------------");


        // 치환 : Replace
        System.out.println("replace: " + str.replace("Programming",
                                                    "Coding"));
        ...
        // 여러가지 메소드를 통해 변화가 발생했더라도 원본은 유지된다.

        System.out.println("---------------------------------------");

        // 구분자로 분리 : split()
        String[] split = str.split(" ");    // 공백문자를 기준으로 문자열을 분리 -> 문자열 배열로 변환
        ...
        // 문자열 Unicode 비교해서 같으면 "0", 작으면 "-1", 크면 "1"
